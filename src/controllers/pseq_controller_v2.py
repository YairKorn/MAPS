...
class PSeqMAC(BasicMAC):
    ...
    def select_actions(self, ep_batch, t_ep, t_env, bs=..., test_mode=False):
        ...
        interaction_cg = self.action_model.update_state(ep_batch["obs"][:, t_ep, 0])

        # build the basic input that common to all the agents #? Consider Conv2D layer and non-flat observation
        # based on _build_inputs of BasicMAC #! I think that this insn't neede because it modeled in action model
        # self.step_inputs = []
        ...
        execute_order = np.random.permutation(interaction_cg) if self.random_ordering else interaction_cg
        ...
        for i in execute_order:
            ...
            obs_batch, obs_probs = self.action_model.get_obs_agent(i)
